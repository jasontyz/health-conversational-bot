...
import diagnosis.APIaccess as apiaccess
from diagnosis import constants
...
def parse_complaint(age, sex, auth_string, case_id, text, context):
...
def mention_as_text(mention):
    ...
    """
    _modality_symbol = {"present": "yes", "absent": "no", "unknown": "?"}
    name = mention["name"]
...
def read_complaints(age, sex, auth_string, case_id, language_model=None):
    ...
    while True:
        portion = read_complaint_portion(age,
                                         sex,
                                         auth_string,
                                         case_id,
                                         context,
...
def conduct_interview(evidence, age, sex, case_id, auth, language_model=None):
    ...
    empty answer."""
    while True:
        resp = apiaccess.call_diagnosis(evidence,
                                        age,
                                        sex,
                                        case_id,
                                        auth,
        ...
        if should_stop_now:
            ...
            triage_resp = apiaccess.call_triage(evidence,
                                                age,
                                                sex,
                                                case_id,
                                                auth,
        ...
        if question_struct['type'] == 'single':
            ...
            if observation_value is not None:
                new_evidence.extend(
                    apiaccess.question_answer_to_evidence(
...
def conduct_interview_single(evidence,
                             age,
                             sex,
                             case_id,
                             auth,
    ...
    if question_struct['type'] == 'single':
        ...
        if observation_value is not None:
            new_evidence.extend(
                apiaccess.question_answer_to_evidence(question_item,
                                                      observation_value))
    else:
        # You'd need a rich UI to handle group questions gracefully.
        # There are two types of group questions: "group_single" (radio
...
def summarise_all_evidence_text(evidence):
    reported = []
    answered = []
    for piece in evidence:
        (reported if piece.get('initial') else answered).append(piece)
    ...
    result = []
    for idx,piece in enumerate(evidence):
        result.append(f"{mention_as_text(piece)}")
    return result
...
def summarise_triage(triage_resp):
    ...
    if teleconsultation_applicable is not None:
        print('Teleconsultation applicable: {}'.format(
            teleconsultation_applicable))
    print()
